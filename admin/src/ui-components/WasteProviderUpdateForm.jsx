...
export default function WasteProviderUpdateForm(props) {
  const {
    id: idProp,
    wasteProvider,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = wasteProviderRecord
      ? { ...initialValues, ...wasteProviderRecord }
      : initialValues;
  ...
  const [wasteProviderRecord, setWasteProviderRecord] =
    React.useState(wasteProvider);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(WasteProvider, idProp)
        : wasteProvider;
      setWasteProviderRecord(record);
    };
    queryData();
  }, [idProp, wasteProvider]);
  React.useEffect(resetStateValues, [wasteProviderRecord]);
  const validations = {
    name: [{ type: "Required" }],
    image: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            WasteProvider.copyOf(wasteProviderRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "WasteProviderUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || wasteProvider)}
